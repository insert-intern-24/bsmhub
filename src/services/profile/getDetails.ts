...
const getDetails = async (profile_id: string): Promise<Details | null> => {
  ...
  const [skill_Datas, link_Datas, competition_Datas, certificate_Datas] =
    await Promise.all([
  ...
  const Detail_Datas = {
    details: [
      ...
      },
      {
        label: '자격증',
        symbol: 'license',
        contents: certificate_Datas.map((item) => ({
      ...
      },
      {
        label: '수상이력',
        symbol: 'prize',
        contents: competition_Datas.map((item) => ({
