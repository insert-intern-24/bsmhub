...
두 서비스가 같은 도메인의 다른 경로에 배포되어 있지만, 서로 다른 Supabase 클라이언트를 사용하여 세션이 공유되지 않는 문제가 발생합니다.
...
1. **bsmhub (현재 앱)**
   - `@supabase/ssr` 사용
   - 서버: 쿠키 기반 인증
   - 클라이언트: localStorage + 쿠키 동기화

2. **admin-web**
   - `@supabase/supabase-js` 사용
   - localStorage만 사용 (쿠키 미사용)

→ admin-web은 쿠키를 읽지 않기 때문에 bsmhub에서 로그인해도 세션을 인식하지 못함

## 해결 방법

### 1. bsmhub (현재 리포지토리) - 완료 ✅

쿠키가 `/` 경로에서 설정되어 `/admin`에서도 접근 가능하도록 명시적으로 설정했습니다.
